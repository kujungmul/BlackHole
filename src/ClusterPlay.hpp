...
public :
	...
	static bool updateBarneshut(int currentIter, nodeCollection* p, exponentVar& expVar, memoryManager* mgr, int nrIteration){
		...
		double XY[DIMENSION] = { 0.0,0.0, };
		double X1Y1[DIMENSION] = {0.0,0.0, };
		double oldXY[DIMENSION] = { 0.0,0.0, };

		for(unsigned int i = 0; i < (*vect).size(); i++){
			double oldEnergy = p->getEnergy((*vect)[i], expVar, octTree);
			double bestDir[DIMENSION] = { 0.0, 0.0, };
	...
	static OctTree* buildOctTree(nodeCollection* ncp, memoryManager* mgr){
		double minPos[DIMENSION] = { 0, };
		double maxPos[DIMENSION] = { 0, };
		double position[DIMENSION] = { 0, };
		double positionTemp[DIMENSION] = { 0, };
